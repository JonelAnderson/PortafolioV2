			...
			<div class="page-body page-body-light pt-3">
				<div class="card card-header-actions">
					<div class="card-header">
						Portafolio
						<a href="{{route('portafolio.create')}}" class="btn btn-primary lift">{{ __('Create New') }}</a>
					</div>
						...
						<table class="table table-sm  table-hover">
							...
							@foreach ($portafolios as $item)
								<tr>
									...
									<td class="text-center"><img src="../images/portfolio/{{$item->photo}}" width="100" heigth="50"></td>
									<td class="text-center">{{$item->description}}</td>
									<td class="text-center">
											<form action="{{ route('portafolio.destroy',$item->id) }}" method="POST" class="formEliminar">
												<a class="btn btn-warning  lift btn-sm" href="{{ route('portafolio.edit',$item->id) }}"><i class="fa fa-fw fa-edit"></i> Edit</a>
												@csrf
												@method('DELETE')
