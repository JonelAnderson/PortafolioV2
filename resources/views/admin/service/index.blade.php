			...
			<div class="page-body page-body-light pt-3">
				<div class="card card-header-actions">
					<div class="card-header">
						Services
						<a href="{{route('service.create')}}" class="btn btn-primary lift">{{ __('Create New') }}</a>
					</div>
						...
						<table class="table table-sm table-hover">
							...
							@foreach ($services as $item)
								<tr>
									...
									<td class="text-center">{{$item->image_service}}</td>
									<td class="text-center">{{$item->description}}</td>
									<td class="text-center">
									    
											<form action="{{ route('service.destroy',$item->id) }}" method="POST" class="formEliminar">
												<a class="btn btn-warning  lift btn-sm" href="{{ route('service.edit',$item->id) }}"><i class="fa fa-fw fa-edit"></i> Edit</a>
												@csrf
												@method('DELETE')
